...
def calculate_auv_acceleration(
    force_magnitude, force_angle, mass=100, volume=0.1, thruster_distance=0.5
):
    ...
    if not all(
        ...
        for val in (force_magnitude, force_angle, mass, volume, thruster_distance)
    ):
        raise ValueError("Invalid input value(s).")
    ...
    if force_angle > (np.pi / 6) or force_angle < (-np.pi / 6):
        raise ValueError(
            "Angle of thruster may not be greater than pi/6 radians in either direction."
        )
...
def calculate_auv_angular_acceleration(
    force_magnitude, force_angle, inertia=1, thruster_distance=0.5
):
    ...
    if not all(
        isinstance(val, (int, float))
        for val in (force_magnitude, force_angle, inertia, thruster_distance)
    ):
    ...
    angular_acceleration = (
        force_magnitude * np.sin(force_angle) * thruster_distance
    ) / inertia
    return angular_acceleration
