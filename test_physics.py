...
class TestPhysics(unittest.TestCase):
    ...
    def test_calculate_auv_acceleration(self):
        self.assertAlmostEqual(
            physics.calculate_auv_acceleration(10, np.pi / 12, 100, 0.1, 0.5),
            0.099999999999,
        )
        ...
        self.assertRaises(
            ValueError, physics.calculate_auv_acceleration, 10, np.pi / 3, 100, 0.1, 0.5
        )
        # Negative mass
        self.assertRaises(
            ValueError,
    ...
    def test_calculate_auv_angular_acceleration(self):
        self.assertAlmostEqual(
            physics.calculate_auv_angular_acceleration(10, np.pi / 4, 1, 0.5),
            14.1421356237,
        )
        self.assertNotEqual(
            physics.calculate_auv_angular_acceleration(10, np.pi / 4, 1, 0.5), 20
        )
        self.assertRaises(
            ValueError,
    ...
    def test_calculate_auv2_acceleration(self):
            ...
            np.all(
                np.isclose(
                    physics.calculate_auv2_acceleration(
                        np.array([[2], [2], [0], [0]]), 0.5, 0.5, 0.5
                    ),
                    np.array([[6.16120922], [3.36588394]]),
                )
            )
        )
        self.assertFalse(
        ...
        self.assertRaises(
            TypeError, physics.calculate_auv2_acceleration, [0, 2, 0, 2], 0.5, 0.5, 0.5
        )
        self.assertRaises(
            ValueError,
            physics.calculate_auv2_acceleration,
    ...
    def test_calculate_auv2_angular_acceleration(self):
        self.assertAlmostEqual(
            physics.calculate_auv2_angular_acceleration(
                np.array([[2], [2], [0], [0]]), 0.5, 2, 2, 100
            ),
            0.0,
        )
        self.assertAlmostEqual(
            physics.calculate_auv2_angular_acceleration(
                np.array([[2], [1], [0], [0]]), 0.5, 2, 2, 100
            ),
            0.027140162009891514,
        )
        self.assertNotEqual(
            physics.calculate_auv2_angular_acceleration(
                np.array([[2], [4], [0], [0]]), 0.5, 2, 2, 100
            ),
            0.027140162009891514,
        )
        self.assertRaises(
